...
function formatRecordsAsTable(records) {
    ...
    const columnWidths = keys.map(key => {
        const maxValueLength = Math.max(key.length, ...records.map(r => String(r.map[key] || '').length));
 ...
 */
function formatRecordsAsHTML(records, queryInfo) {
    if (records.length === 0) {
 ...
 */
function runQueryCommand(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let outputFormat = metadata.output || 'webview';
